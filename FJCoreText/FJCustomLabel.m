...
#import "FJCustomLabel.h"

static NSString *kFJImageInfoAttributeName = @"kFJImageInfoAttributeName";
static NSString *kFJImageAttributeName = @"kFJImageAttributeName";
static NSString *kFJImageVerticalOffsetAttributeName = @"kFJImageVerticalOffsetAttributeName";
...
+(void)initialize{
	...
	if(!hashtagRegEx){
		NSError *error = NULL;
		hashtagRegEx = [NSRegularExpression regularExpressionWithPattern:@"(#.+?)(?:[^0-9A-Za-z_\\.]|$)" options:NSRegularExpressionCaseInsensitive error:&error];
		if(!hashtagRegEx){
			NSLog(@"error creating regex: %@", error);
		}
	}
...
-(void)setShouldLinkTypes:(kFJShouldLink)_shouldLinkTypes{
	shouldLinkTypes = _shouldLinkTypes;
	self.userInteractionEnabled = YES;
}
-(void)setDefaults{
	self.backgroundColor = [UIColor whiteColor];		
	[self setDefaultStyle:[FJCustomLabelStyle new]];
...
-(void)setDefaultStyle:(FJCustomLabelStyle *)style{
	[self setStyle:style forKey:FJCustomLabelStyleDefaultKey];
}
-(FJCustomLabelStyle *)defaultStyle{
	if([self.styles objectForKey:FJCustomLabelStyleDefaultKey]){
		return [self.styles objectForKey:FJCustomLabelStyleDefaultKey];
	}else{
		return [FJCustomLabelStyle new];
	}
}
...
//	NSLog(@"customlabel - create - %@", text);
	...
	FJCustomLabelStyle *defaultStyle = [self defaultStyle];
	...
	if(self.shouldBoldAtNames && (self.shouldLinkTypes & kFJShouldLinkUsernames) ){
					...
					if(linkColor){
						...
						if(highlightedTextIndex != NSNotFound){
							if(highlightedTextIndex >= range.location && highlightedTextIndex < range.location+range.length){
								highlightedTextType = kFJTextTypeUsername;
						...
						FJCustomLabelStyle *boldStyle = [self.styles objectForKey:FJCustomLabelStyleBoldKey];
    ...
    if(kFJShouldLinkHashtags){
		[hashtagRegEx enumerateMatchesInString:self.text options:0 range:NSMakeRange(0, [self.text length]) usingBlock:^(NSTextCheckingResult *match, NSMatchingFlags flags, BOOL *stop){
			...
			if(match.numberOfRanges > 1){
				...
				if(range.length > 1){
					...
					if(linkColor){
						...
						if(highlightedTextIndex != NSNotFound){
							if(highlightedTextIndex >= range.location && highlightedTextIndex < range.location+range.length){
								highlightedTextType = kFJTextTypeHashtag;
								tehLinkColor = activeLinkColor;
								highlightedText = [cleanText substringWithRange:NSMakeRange(range.location, range.length)];
							}
						}
	...
	if(self.shouldLinkTypes & kFJShouldLinkURLs){
		NSError *error = NULL;
			...
			if(linkColor){
				...
				if(highlightedTextIndex != NSNotFound){
					if(highlightedTextIndex >= matchRange.location && highlightedTextIndex < matchRange.location+matchRange.length){
						highlightedTextType = kFJTextTypeLink;
						tehLinkColor = activeLinkColor;
...
-(void)resetHighlightedText{
	highlightedTextIndex = NSNotFound;
	highlightedText = nil;
	highlightedTextType = kFJTextTypeNone;
	[self createAttributedString];
	[self setNeedsDisplay];
}
