...
#import "FJKeychainUtils.h"
#import <Security/Security.h>

static NSString *FJKeychainUtilsErrorDomain = @"FJKeychainUtilsErrorDomain";

#if __IPHONE_OS_VERSION_MIN_REQUIRED < 30000 && TARGET_IPHONE_SIMULATOR
@interface FJKeychainUtils (PrivateMethods)
+ (SecKeychainItemRef) getKeychainItemReferenceForUsername: (NSString *) username andServiceName: (NSString *) serviceName error: (NSError **) error;
@end
#endif

@implementation FJKeychainUtils

#if __IPHONE_OS_VERSION_MIN_REQUIRED < 30000 && TARGET_IPHONE_SIMULATOR

+ (NSString *) getPasswordForUsername: (NSString *) username andServiceName: (NSString *) serviceName error: (NSError **) error {
	if (!username || !serviceName) {
		*error = [NSError errorWithDomain: FJKeychainUtilsErrorDomain code: -2000 userInfo: nil];
		return nil;
	}
	
	SecKeychainItemRef item = [FJKeychainUtils getKeychainItemReferenceForUsername: username andServiceName: serviceName error: error];
	...
	if (status != noErr) {
		*error = [NSError errorWithDomain: FJKeychainUtilsErrorDomain code: status userInfo: nil];
		return nil;
    }
...
+ (NSString *) getPasswordForUsername: (NSString *) username andServiceName: (NSString *) serviceName error: (NSError **) error {
	...
	if (status != noErr) {
		if (status == errSecItemNotFound) {
			...
			if (error != nil) {
				*error = [NSError errorWithDomain: FJKeychainUtilsErrorDomain code: -1999 userInfo: nil];
			}
		}
		else {
...
{		
	...
	NSString *existingPassword = [FJKeychainUtils getPasswordForUsername: username andServiceName: serviceName error:&getError];
